theme_sopi <- function(family = "Calibri", base_size = 10.5, ...) {
  ...
  default_theme <- list(
    text = if (!is.null(family)) element_text(family = family) else element_text(),
    axis.line = element_line(color = "#dad9d9"),
    axis.ticks = element_line(color = "#dad9d9"),
  ...
  # Merge, giving priority to user-supplied arguments
  merged_theme <- modifyList(default_theme, user_theme)
  theme_minimal(base_size = base_size) + do.call(theme, merged_theme)
...
get_nice_breaks <- function(max_val, min_breaks = 3, max_breaks = 6) { 
  ...
  # allow smaller scales if range is small
  ...
  for (s in steps) {
    ...
    if (length(breaks) >= min_breaks && length(breaks) <= max_breaks) {    
      ...
      # derive label accuracy from step
      accuracy <- if (step < 1) {
