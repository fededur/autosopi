plot_milk_production <- function(
  ...
  average_label <- paste0(
    min(complete_seasons) - 1,
    "/",
    substr(min(complete_seasons), 3, 4),
    "-",
    substr(max(complete_seasons), 3, 4),
    " average"
  )
  ...
  legend_items_line <- tibble::tibble(
    legend_item = factor(rep(average_label, 2), levels = fill_levels),
  ...
  ggplot() +
    ...
    scale_colour_manual(
      values = setNames("#6d6e70", average_label),
      breaks = average_label,
      name = NULL
    ) +
